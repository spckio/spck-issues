...
/**
 * @class LFUCache
 * @classdesc Extremely minimal implementation of LFU caching. Designed to improve performance of high frequency calls.
 */
function LFUCache(maxSize, evictHitRatio) {
  var _keys = 0
  var _maxHits = 1
  var _cache = {},
      _hits = {}
  ...
  /**
   * Reverse lookup a key by its value
   * @param {*} value
   */
  function find(value) {
    return Object.keys(_cache).find(k => _cache[k] === value)
  }
   ...
   */
  function get(key) {
    var hits = ++_hits[key] || undefined
    return hits && (_maxHits = hits > _maxHits ? hits : _maxHits) && _cache[key]
  }
   ...
   * @param {string} key
   */
  function rename(oldkey, key) {
    _cache[key] = _cache[oldkey]
    _hits[key] = _hits[oldkey]
  ...
  function set(key, value) {
    _keys++
    (_keys > maxSize && _evict())
    _hits[key] = 1
    return _cache[key] = value
  }

  function _evict() {
    var newHits = {}, newCache = {}
    _keys = 0
    Object.keys(_hits).forEach(key => {
      var hits = _hits[key]
      if (hits && hits/_maxHits > evictHitRatio) {
        _keys++
        newHits[key] = 1
...
function GitFileSystemClass(fs) {
  ...
  function saveBase64(name, buffer) {
    if (AndroidRef && AndroidRef.saveBase64) {
      // FileSaver saveAs doesn't work on Android
      AndroidRef.saveBase64(Buffer.from(buffer).toString('base64'), name)
    } else {
  ...
  function mkdir(path) {
    ...
    return promise.then(created => {
      if (created) {
        _readdirDeepCache.clear()
        path.split('/').slice(0, -1).forEach((name, index, array) => {
          _readdirCache.del(array.slice(0, index + 1).join('/'))
        })
        return observable.run({
          action: 'mkdir',
          path,
