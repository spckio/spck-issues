...
function GitFileSystemClass(fs) {
  ...
  if (GlobalEmitter) {
    this.observable.subscribe(args => GlobalEmitter.emit('::update', args))
  }
  ...
  function lock(filename, triesLeft) {
    ...
    if (triesLeft === 0) {
      return Promise.reject({
        ...
        filename,
        code: GlobalErrorCodes.AcquireLockFileFail
      })
    }
    var lockpath = filename + '.lock'
    return (fs.supported ? fs.mkdirp(lockpath) : _mkdir(lockpath))
      .catch(() => {
  ...
  function unlock(filename, delayRelease, tries) {
    delayRelease = delayRelease || 50
    if (delayedReleases[filename]) {
      throw new Error(`Double release lock file failed: ${filename}`)
    }
    ...
    delayedReleases.set(
      ...
      setTimeout(() => {
        ...
        return (fs.supported ?
          fs.remove(filename) :
          store.folders.where('path').equals(filename).delete()
        ).catch(err => {
          $warn('Unable to unlock', filename, 'due to:', err)
          // Attempt again if failed, up to 3 times
          tries = tries || 0
          if (tries < 3) unlock(filename, delayRelease, tries + 1)
        })
      }, delayRelease)
    )
  }
